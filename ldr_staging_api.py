...
class Techmds(Resource):
    def get(self, stage_id, segment_id, ms_id):
        ...
        try:
            ...
            r['segment_id'] = segment.identifier
            r['materialsuite_id'] = ms_id
            r['tech_mds'] = [x.item_name for x in ms.technicalmetadata_list]
            return jsonify(
                APIResponse("success", data=r).dictify()
            )
        except Exception as e:
    ...
    def get(self, stage_id, segment_id, ms_id, techmd_id):
        ...
        try:
            ...
            if len(ms) != 1:
                raise ValueError("Bad MaterialSuite Identifier")
            ms = ms[0]
            techmd = [x for x in ms.technicalmetadata_list if x.item_name == techmd_id]
            if len(techmd) != 1:
...
api.add_resource(Presform, '/stages/<string:stage_id>/<string:segment_id>/<path:ms_id>/presforms/<string:presform_id>')
api.add_resource(Techmds, '/stages/<string:stage_id>/<string:segment_id>/<path:ms_id>/techmds')
api.add_resource(Techmd, '/stages/<string:stage_id>/<string:segment_id>/<path:ms_id>/techmds/<string:techmd_id>')
